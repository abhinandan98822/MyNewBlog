...
def SignUp(request):
    ...
    if request.method == 'POST':
        ...
        if len(password) < 8:
            ...
            messages.error(request,"*Atleast One Capital letter required in password")
            ...
            send_mail(subject,message,email_from,recipient_list,html_message=email_message,fail_silently=True)
...
def Forgot_Password(request):
    if request.method=="POST":
        ...
        if user:
            ...
            messages.error(request,"User with this email address does not exist,enter valid email address.")
...
def share_blog(request, blog_id):
    ...
    if request.method == 'POST':
        ...
        send_mail(subject, "", from_email, recipient_list, html_message=email_message,fail_silently=True)
